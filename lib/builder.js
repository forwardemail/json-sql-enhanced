...
Builder.prototype._handleBufferValue = function (value) {
  // If bufferAsNative is enabled, pass Buffer through without conversion
  // This allows better-sqlite3 to store as native BLOB (more efficient)
  if (this.options.bufferAsNative) {
    if (this.options.separatedValues) {
      const placeholder = this._getPlaceholder();
      if (this.options.namedValues) {
        this._values[placeholder] = value;
    ...
    // Cannot inline a Buffer without separatedValues
    throw new Error('bufferAsNative requires separatedValues to be enabled');
  }

  // Default behavior: convert to hex string for backwards compatibility
